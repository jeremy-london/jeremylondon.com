...
Depth: systems-essay
Information map:
- the concrete tool calling problem
- the example surface: a model producing a tool call that is syntactically valid but operationally incomplete
...
Tool calling works best when the model, tool schema, identity, and application agree on a clear contract.

The hard part of tool calling is not making a prototype move. Prototypes move. They call tools, retrieve documents, choose models, draw diagrams, and produce tool calling output that looks close enough to keep going. The hard part is deciding what the tool calling system is allowed to do after it produces something plausible.

I think about tool calling as a control problem more than a prompt problem. The prompt matters for tool calling, but the prompt is only one component inside a larger machine: context construction, policy, routing, tool execution, verification, storage, approval, and replay. If those pieces are missing, the tool calling system may still look impressive in a demo. It will just be unpleasant to operate.

## The problem

The concrete surface here is a model producing a tool call that is syntactically valid but operationally incomplete. That tool calling surface creates pressure because it crosses from language into work. Once a tool calling system can affect files, route requests, inspect user state, call tools, spend money, or produce security-relevant conclusions, "the model answered" is no longer the end of the story.

The tool calling system needs to answer different questions:
...
The obvious tool calling solution is to add a better model and a thicker prompt. That helps tool calling, but it does not solve the operating problem. A stronger model can still break tool calling: call the wrong tool, use stale context, skip a permission boundary, or produce a technically valid answer that is useless to the person who has to act on it.

The second obvious tool calling solution is to put a human approval button at the dangerous step. That also helps tool calling, but only if the approval has context. A tool calling button that says "approve" without showing the diff, tool arguments, policy boundary, expected side effect, and rollback path is not governance. It is a liability with a nicer label.

For tool calling, I would treat approvals as part of the data flow. The approval is not a modal. It is a signed tool calling decision attached to the run record.

## Constraints

A production tool calling version has to live with several constraints at once.

The first is latency. Verification cannot be so slow that everyone disables it. The second is cost. Running a frontier model as judge for every small tool calling step may be correct and still unaffordable. The third is privacy. For tool calling, context windows are tempting places to dump everything, but credential material, user state, and internal documents need boundaries. The fourth is replay. If a tool calling failure cannot be replayed, it will be explained with folklore.
...
The operating loop is where tool calling can become reliable or collapse into a pile of impressive transcripts. I would make every run produce a record that can be inspected by a human, replayed by a test harness, and summarized by an eval dashboard. Those are three different consumers, so the record cannot be only prose.
...
For tool calling, I would separate three kinds of evidence:
...
The measurement surface for tool calling is reproduction time, failing example coverage, queue age, and rollback confidence. I would not roll that into one success score. One number will hide the shape of the system.
...
I do not think there is one clean answer for tool calling. The shape depends on the cost of failure, the sensitivity of the data, the reversibility of the action, and the review capacity of the team.
...
That is the part I find interesting. The future of tool calling is not a chat box with more buttons. It is a work surface with memory, scope, evidence, review, revocation, and enough humility to stop when it cannot prove what it just did.
