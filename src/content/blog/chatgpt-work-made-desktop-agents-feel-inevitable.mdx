...
That sounds like product positioning until you ask what has to be true for the work to be trusted.

A chat assistant can get by with a transcript and a send button. A desktop agent needs more structure than that. It needs scoped access, visible state, approvals with real context, verifiers that can disagree with the agent, and records that survive after the conversation scrolls away.
...
## a run is the thing you can inspect
...
I want the platform to think in objects like this:
...
That is the part I care about. The product is not "chat with tools." The product is a structured work session with a beginning, a boundary, and an exit.

## desktop context is personal before it is productive
...
The user should be able to see those grants without opening a policy document. More importantly, the runtime should enforce them without relying on the model to remember the rules.
...
## the verifier should not share the generator's job
...
## receipts are how runs stay inspectable
