...
<CodeBlock client:only="react" code={`# Compute the gradient of the loss with respect to z3 (Layer 3 pre-activation)
...
<CodeBlock client:only="react" code={`# Compute the gradients of the loss with respect to Layer 3 weights and biases
...
The code lines above marks the start of backpropagation, where it calculates the gradient of the loss concerning the final layer's pre-activation values. This gradient shows how much the loss would change if we tweaked those values.
