...
- the example surface: an evaluation suite that catches a regression before a model swap quietly changes product behavior
...
The concrete surface here is an evaluation suite that catches a regression before a model swap quietly changes product behavior. That evals surface creates pressure because it crosses from language into work. Once a evals system can affect files, route requests, inspect user state, call tools, spend money, or produce security-relevant conclusions, "the model answered" is no longer the end of the story.
