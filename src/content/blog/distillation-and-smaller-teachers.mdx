...
Depth: systems-essay
Information map:
- the concrete distillation problem
- the example surface: a practical note on using smaller models for focused jobs instead of sending every request to the largest model
...
A practical note on using smaller models for focused jobs instead of sending every request to the largest model.

The hard part of distillation is not making a prototype move. Prototypes move. They call tools, retrieve documents, choose models, draw diagrams, and produce distillation output that looks close enough to keep going. The hard part is deciding what the distillation system is allowed to do after it produces something plausible.

I think about distillation as a control problem more than a prompt problem. The prompt matters for distillation, but the prompt is only one component inside a larger machine: context construction, policy, routing, tool execution, verification, storage, approval, and replay. If those pieces are missing, the distillation system may still look impressive in a demo. It will just be unpleasant to operate.

## The problem

The concrete surface here is a practical note on using smaller models for focused jobs instead of sending every request to the largest model. That distillation surface creates pressure because it crosses from language into work. Once a distillation system can affect files, route requests, inspect user state, call tools, spend money, or produce security-relevant conclusions, "the model answered" is no longer the end of the story.

The distillation system needs to answer different questions:
...
The obvious distillation solution is to add a better model and a thicker prompt. That helps distillation, but it does not solve the operating problem. A stronger model can still break distillation: call the wrong tool, use stale context, skip a permission boundary, or produce a technically valid answer that is useless to the person who has to act on it.

The second obvious distillation solution is to put a human approval button at the dangerous step. That also helps distillation, but only if the approval has context. A distillation button that says "approve" without showing the diff, tool arguments, policy boundary, expected side effect, and rollback path is not governance. It is a liability with a nicer label.

For distillation, I would treat approvals as part of the data flow. The approval is not a modal. It is a signed distillation decision attached to the run record.

## Constraints

A production distillation version has to live with several constraints at once.

The first is latency. Verification cannot be so slow that everyone disables it. The second is cost. Running a frontier model as judge for every small distillation step may be correct and still unaffordable. The third is privacy. For distillation, context windows are tempting places to dump everything, but credential material, user state, and internal documents need boundaries. The fourth is replay. If a distillation failure cannot be replayed, it will be explained with folklore.

The trust boundary is where the model stops and the serving system starts. I would make that boundary explicit in the request envelope rather than burying it in code comments.
...
The operating loop is where distillation can become reliable or collapse into a pile of impressive transcripts. I would make every run produce a record that can be inspected by a human, replayed by a test harness, and summarized by an eval dashboard. Those are three different consumers, so the record cannot be only prose.
...
For distillation, I would separate three kinds of evidence:
...
The obvious failure is a model choice is treated as isolated even though the surrounding system made the decision. The less obvious failures are more interesting.
...
The measurement surface for distillation is task success, latency budget, memory footprint, batch behavior, and quality by route. I would not roll that into one success score. One number will hide the shape of the system.
...
I do not think there is one clean answer for distillation. The shape depends on the cost of failure, the sensitivity of the data, the reversibility of the action, and the review capacity of the team.
...
That is the part I find interesting. The future of distillation is not a chat box with more buttons. It is a work surface with memory, scope, evidence, review, revocation, and enough humility to stop when it cannot prove what it just did.
