...
const BackPropagation = () => {
  ...
  }, [combineBiasAndWeights, extractValues])
  ...
  const handleMatrixW1Change = (newMatrixW1) => {
    ...
    const biasesString = `b1 = np.array([${newBiases.join(', ')}])`
  ...
  const handleMatrixW2Change = (newMatrixW2) => {
    ...
    // Construct the biases string for numpy
    const biasesString = `b2 = np.array([${newBiases.join(', ')}])`
  ...
  const handleMatrixW3Change = (newMatrixW3) => {
    ...
    // Construct the biases string for numpy
    const biasesString = `b3 = np.array([${newBiases.join(', ')}])`
  ...
  return (
    <div
      ...
      className="grid-rows-auto mb-4 grid hidden grid-cols-4 place-items-center gap-2 rounded-md bg-[#e9e9e9] p-4 text-[#d0d0d0] sm:gap-4 md:grid-cols-[2fr_1fr_1fr_1fr_0.8fr] dark:bg-[#292929] dark:text-[#f5f2f0]"
    >
      {/* Input Layer */}
