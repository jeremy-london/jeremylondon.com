        ...
        <Fragment key={`row-${rowIndex}`}>
          {rowValues.map((value, columnIndex) => (
            <input
              ...
              type="text"
              inputMode={
                /Android/i.test(navigator.userAgent) ? 'decimal' : 'numeric'
              }
              pattern="-?[0-9]*" // Allows only numbers, but doesn't strictly enforce it
              className="rounded-md border border-solid border-gray-400 text-center text-sm text-black disabled:border-gray-400 disabled:bg-[#d0d0d0] sm:text-base dark:border-gray-200 dark:text-white disabled:dark:border-gray-600 disabled:dark:bg-[#222222]"
              value={
                temporaryValues[`${rowIndex}-${columnIndex}`] ??
                value.toString()
              } // Convert value to string to handle empty and '0' values
              onChange={(e) =>
