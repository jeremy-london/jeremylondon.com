...
2. **Direction questions answered** (native-image.md Step A)
3. **Palette confirmed** (native-image.md Step B)
4. **One mock direction approved or delegated** (native-image.md Step D)

You must stop at every gate. **Shape confirmation alone is NOT a green light to start coding.** It is the green light to begin native-image.md Step A. Compressing gates 2 through 4 because the shape brief felt complete is the dominant failure mode of this flow.
...
If the harness has **native image generation**, this step is mandatory. **Stop and load [native-image.md](native-image.md)**. It covers palette generation, mock exploration, the approval loop, mock-fidelity inventory, and asset slicing via the `impeccable_asset_producer` subagent. Follow Steps A-F in that file, then return here for Step 4.
...
Look at what you built like a designer would. Your eyes are whatever the harness gives you: Chrome DevTools MCP, a connected browser, a screenshotting tool, or asking the user. Use them for responsive testing (mobile, tablet, desktop minimum) and general visual validation.
