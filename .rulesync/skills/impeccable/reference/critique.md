...
2. **Compute the slug**:
   ...
   ```bash
   node .rulesync/skills/impeccable/scripts/critique-storage.mjs slug "<resolved-path-or-url>"
   ```
...
```bash
node .rulesync/skills/impeccable/scripts/detect.mjs --json [target]
```
...
1. Create a fresh tab and navigate. Prefer Chrome DevTools MCP or the harness's native/browser-canvas screenshot path; only fall back to a custom script when no browser automation tool is exposed.
2. Preflight mutable injection by setting `document.title` and appending a `<script>` tag. Read-only evaluate APIs do not count.
3. If mutation is unavailable, skip live server, browser presentation, and injection; report fallback signal.
4. If mutation is available, start `node .rulesync/skills/impeccable/scripts/live-server.mjs --background`, present the browser if supported, label `[Human]`, scroll top, inject `http://localhost:PORT/detect.js`, wait 2-3 seconds, read `impeccable` console messages, then stop the live server.
...
Auto-select 2-3 personas most relevant to this interface type (use the selection table in the reference). If `.rulesync/settings.json` contains a `## Design Context` section from `impeccable init`, also generate 1-2 project-specific personas from the audience/brand info.
...
2. **Pass the structured metadata** through `IMPECCABLE_CRITIQUE_META` (JSON), then run the write command:
   ...
   IMPECCABLE_CRITIQUE_META='{"target":"<user phrasing>","total_score":<n>,"p0_count":<n>,"p1_count":<n>}' \
     node .rulesync/skills/impeccable/scripts/critique-storage.mjs write <slug> <body-file>
   ```
...
4. **Read the trend** for context:
   ...
   ```bash
   node .rulesync/skills/impeccable/scripts/critique-storage.mjs trend <slug> 5
   ```
...
If `.rulesync/settings.json` contains a `## Design Context` section (generated by `impeccable init`), derive 1-2 additional personas from the audience and brand information:
