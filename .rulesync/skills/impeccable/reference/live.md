...
1. `live.mjs`: boot. If the request names or implies a file, route, or app inside a monorepo, infer the concrete path and run `node .rulesync/skills/impeccable/scripts/live.mjs --target <path>` instead; then run the rest of this live session from the returned `projectRoot`.
...
- If your harness reliably returns background-process stdout to this session, run a one-shot background poll and restart it after each event.
- If background-process stdout is unreliable or unavailable, run the poll in the foreground.
- Do not use stream mode unless this harness has been measured to surface incremental stdout quickly.
...
```bash
node .rulesync/skills/impeccable/scripts/live.mjs
```
...
```text
LOOP:
  node .rulesync/skills/impeccable/scripts/live-poll.mjs   # default long timeout; no --timeout=
...
```text
node .rulesync/skills/impeccable/scripts/live-poll.mjs --stream   # stays running; one JSON line per event
...
```bash
node .rulesync/skills/impeccable/scripts/live-status.mjs
node .rulesync/skills/impeccable/scripts/live-resume.mjs --id SESSION_ID
node .rulesync/skills/impeccable/scripts/live-complete.mjs --id SESSION_ID
```
...
```bash
node .rulesync/skills/impeccable/scripts/live-insert.mjs --id EVENT_ID --count EVENT_COUNT --position after \
...
```bash
node .rulesync/skills/impeccable/scripts/live-wrap.mjs --id EVENT_ID --count EVENT_COUNT --element-id "ELEMENT_ID" --classes "class1,class2" --tag "div" --text "TEXT_SNIPPET"
```
...
```bash
node .rulesync/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID done --file RELATIVE_PATH
```
...
```bash
node .rulesync/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID error "Short reason"
```
...
```bash
node .rulesync/skills/impeccable/scripts/live-poll.mjs --reply EVENT_ID steer_done ["Optional short note for a browser toast"]
```
...
```bash
node .rulesync/skills/impeccable/scripts/live-server.mjs stop
```
...
```bash
node .rulesync/skills/impeccable/scripts/detect-csp.mjs
```
