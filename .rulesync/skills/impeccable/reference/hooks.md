...
The hook runs the impeccable design detector on direct file edits to design-relevant files (`.tsx`, `.jsx`, `.html`, `.vue`, `.svelte`, `.astro`, `.css`, `.scss`, `.sass`, `.less`, `.ts`, `.js`). Post-edit harnesses push a short system reminder into the agent's context after the edit; findings get a correction prompt, pending issues get a re-nudge, and clean UI-ish files get a short ack unless quiet mode is on (`hook.quiet` in config). Pre-edit harnesses can block bad proposed writes before they land and stay silent when they allow a clean write.
...
Supported harnesses use their generated local hook manifests. Prefer project-local, generated hook config from `.rulesync`; do not hand-maintain harness-specific hook files as the source of truth.
...
2. Invoke the admin script and pass the user's output through verbatim:
   ...
   ```bash
   node .rulesync/skills/impeccable/scripts/hook-admin.mjs <action> [args...]
   ```
...
```bash
node .rulesync/skills/impeccable/scripts/hook-admin.mjs ignore-value overused-font Inter --shared --reason "User confirmed Inter is intentional"
```

Example intentional motion exception:

```bash
node .rulesync/skills/impeccable/scripts/hook-admin.mjs ignore-value bounce-easing bounce-ball --shared --reason "User confirmed ball bounce animation is intentional"
```

Example whole-rule font exception:

```bash
node .rulesync/skills/impeccable/scripts/hook-admin.mjs ignore-rule overused-font --all-values --reason "User asked to ignore overused fonts generally"
```

Example file-scoped exception:

```bash
node .rulesync/skills/impeccable/scripts/hook-admin.mjs ignore-file "src/legacy/Card.tsx"
```
...
- Pre-edit harnesses can block a proposed write when the detector finds a real issue. Post-edit harnesses emit a reminder instead. Disabling stops both blocking and reminders.
- The hook is bundled with the Impeccable skill and installed through project-local generated manifests. Approve or enable hooks using the current harness's normal hook UI when prompted.
